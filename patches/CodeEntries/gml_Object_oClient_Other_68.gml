        ...
        switch (msgid)
        {
            ...
            case 100:
                ...
                if (global.clientID != clientID)
                {
                    if (findsocket < 0)
                    {
                        if (proc_net_room_matches(clientID, clientRoom) || (!global.proc_active && (clientRoom == 6 || clientRoom == 10 || clientRoom == 9 || clientRoom == 11 || clientRoom == 16)))
                        {
            ...
            case 101:
                ...
                var findsocket = ds_list_find_index(roomList, clientID);
                if (proc_net_room_matches(clientID, clientRoom) && global.clientID != clientID)
                {
                    sameRoom = 1;
                }
                else if (!proc_net_room_matches(clientID, clientRoom) && global.clientID != clientID && (global.proc_active || (clientRoom != 6 && clientRoom != 10 && clientRoom != 9 && clientRoom != 11 && clientRoom != 16)))
                {
            ...
            case 33:
                ...
                if (proc_net_room_matches(mirror, sentRoom) && !sax)
                {
            ...
            case 69:
                ...
                for (var i = 0; i < ds_list_size(roomListData); i++)
                {
                    ...
                    if (clientID == arrID)
                    {
                        ...
                        if (proc_net_room_matches(clientID, arrRoom))
                        {
                ...
                break;
            case 121:
                global.proc_net_seed = buffer_read(_buffer, buffer_f64);
                global.proc_net_room = buffer_read(_buffer, buffer_f64);
                global.proc_net_depth = buffer_read(_buffer, buffer_u32);
                global.proc_net_entry_kind = buffer_read(_buffer, buffer_u8);
                global.proc_net_entry_side = buffer_read(_buffer, buffer_u8);
                global.proc_net_revision = buffer_read(_buffer, buffer_u32);
                if (!ds_exists(global.proc_open_locks, ds_type_map))
                {
                    global.proc_open_locks = ds_map_create();
                }
                var procOpenCount = buffer_read(_buffer, buffer_u16);
                for (var procOpenIndex = 0; procOpenIndex < procOpenCount; procOpenIndex++)
                {
                    var procOpenRoom = buffer_read(_buffer, buffer_f64);
                    var procOpenKey = string(procOpenRoom);
                    if (!ds_map_exists(global.proc_open_locks, procOpenKey))
                    {
                        ds_map_add(global.proc_open_locks, procOpenKey, 1);
                    }
                }
                global.proc_net_seed_ready = 1;
                global.proc_net_waiting = 0;
                break;
            case 123:
                var procOpenRoom = buffer_read(_buffer, buffer_f64);
                if (!ds_exists(global.proc_open_locks, ds_type_map))
                ...
                {
                    if (global.proc_room_index == procOpenRoom && lock_active)
                    {
                        lock_active = 0;
                        proc_lock_clear(lock_ids);
                        popup_text_ext("LOCKDOWN SINCRONIZADO - PORTAS ABERTAS", 180);
                    }
                }
                break;
            case 127:
                var procPeerID = buffer_read(_buffer, buffer_u8);
                var procPeerRoom = buffer_read(_buffer, buffer_f64);
                ...
                break;
            case 129:
                var procDeadRoom = buffer_read(_buffer, buffer_f64);
                var procDeadID = buffer_read(_buffer, buffer_u8);
                if (global.proc_active && procDeadRoom == global.proc_room_index)
                {
                    with (oEnemy)
                    {
                        if (proc_generated && proc_room_key == procDeadRoom && proc_spawn_index == procDeadID && object_index != oMonster)
                        {
                            instance_destroy();
                        }
                    }
                ...
                break;
            case 131:
                var procMonsterRoom = buffer_read(_buffer, buffer_f64);
                var procMonsterID = buffer_read(_buffer, buffer_u8);
                var procMonsterOwner = buffer_read(_buffer, buffer_u8);
                var procMonsterState = buffer_read(_buffer, buffer_u8);
                var procMonsterX = buffer_read(_buffer, buffer_s16);
                var procMonsterY = buffer_read(_buffer, buffer_s16);
                var procMonsterHP = buffer_read(_buffer, buffer_s16);
                var procMonsterDead = buffer_read(_buffer, buffer_u8);
                if (global.proc_active && procMonsterRoom == global.proc_room_index)
                {
                    with (oMonster)
                    {
                        if (proc_generated && proc_room_key == procMonsterRoom && proc_spawn_index == procMonsterID)
                        {
                            proc_net_owner = procMonsterOwner;
                            proc_net_timeout = 120;
                            if (procMonsterDead)
                            {
                                instance_destroy();
                            }
