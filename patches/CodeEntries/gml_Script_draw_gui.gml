if (global.classicmode == 0 && global.opshowhud)
{
    ...
    if (global.maxmissiles > 0)
    {
        ...
        {
            draw_background(bgGUIMsl, xoff, ammo_y);
        }
        else
        {
            draw_background(bgGUISmsl, xoff, ammo_y);
        }
        draw_text(0 + xoff + 19, ammo_y + 7, string(global.missiles));
        mslspr = 504;
        ...
        if (global.opmslstyle == 0)
        {
            ...
            {
                draw_sprite(mslspr, 0, 0 + xoff + 1, ammo_y + 4);
            }
            if (global.currentweapon == 1 && oCharacter.state != 23 && oCharacter.state != 24 && oCharacter.state != 27 && oCharacter.state != 54 && oCharacter.state != 55 && oCharacter.sjball == 0)
            {
                ...
                {
                    draw_sprite(mslspr, 1, 0 + xoff + 1, ammo_y + 4);
                }
                if (oCharacter.armmsl == 1)
                {
                    draw_sprite(mslspr, 2, 0 + xoff + 1, ammo_y + 4);
                }
            }
        }
    ...
    {
        draw_background(bgGUISmsl, xoff, ammo_y);
        draw_text(xoff + 19, ammo_y + 7, string(global.smissiles));
        if (global.opmslstyle == 0)
        {
            ...
            {
                draw_sprite(sGUISMissile, 0, xoff + 1, ammo_y + 4);
            }
            if (global.currentweapon == 2 && oCharacter.state != 23 && oCharacter.state != 24 && oCharacter.state != 27 && oCharacter.state != 54 && oCharacter.state != 55 && oCharacter.sjball == 0)
            {
                if (oCharacter.armmsl == 0)
                {
                    draw_sprite(sGUISMissile, 1, xoff + 1, ammo_y + 4);
                }
                if (oCharacter.armmsl == 1)
                {
                    draw_sprite(sGUISMissile, 2, xoff + 1, ammo_y + 4);
                }
            }
        }
        if (global.opmslstyle == 1)
    ...
    {
        draw_background(bgGUISmsl, xoff, ammo_y);
        draw_text(xoff + 19, ammo_y + 7, string(global.pbombs));
        if (global.opmslstyle == 0)
        {
        ...
        if (global.saxmode)
        {
            draw_sprite(sPBombCooldownOverlay, 0, xoff + 2, ammo_y + 17);
            var scaleMult = global.pbombCooldown / global.pbombCooldownMax;
            if (global.pbombs != 0)
            {
                draw_sprite_ext(sPBombCooldownFull, 0, xoff + 2, ammo_y + 17, min(scaleMult, 1), 1, 0, c_white, 1);
            }
        }
    }
