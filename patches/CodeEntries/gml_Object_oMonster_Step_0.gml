if (myid >= 0 && myid <= 98 && global.metdead[myid] == 1 && !dead)
{
    dead = 1;
    hp = 0;
    if (hp <= 0)
...
{
    if (proc_net_owner >= 0 && proc_net_owner != global.clientID)
    {
        proc_net_timeout -= 1;
        if (proc_net_timeout <= 0)
        {
            proc_net_owner = -1;
        }
    }
    if (state >= 1 && proc_net_owner < 0)
    {
        proc_net_owner = global.clientID;
        proc_net_timer = 0;
    }
    if (proc_net_owner >= 0 && proc_net_owner != global.clientID)
    {
        ...
        fangs_frame = anim_frame_cycle(648, fangs_frame, fangs_frspd);
        i += 0.02;
        j = sin(i) * 0.1;
...
if (state == 0)
{
    scale = 0.5;
    image_blend = c_black;
    var _proc_monster_wake = point_distance(x, y, oCharacter.x, oCharacter.y - 16) < 90;
    if (!_proc_monster_wake && proc_generated && global.proc_active && instance_exists(oClient))
    {
        for (var _proc_monster_peer_i = 0; _proc_monster_peer_i < ds_list_size(oClient.posData); _proc_monster_peer_i++)
        {
            var _proc_monster_peer = ds_list_find_value(oClient.posData, _proc_monster_peer_i);
            var _proc_monster_peer_id = _proc_monster_peer[0];
            var _proc_monster_peer_key = string(_proc_monster_peer_id);
            if (_proc_monster_peer_id != global.clientID && ds_exists(global.proc_peer_rooms, ds_type_map) && ds_map_exists(global.proc_peer_rooms, _proc_monster_peer_key))
            {
                if (ds_map_find_value(global.proc_peer_rooms, _proc_monster_peer_key) == global.proc_room_index)
                {
                    if (point_distance(x, y, _proc_monster_peer[1], _proc_monster_peer[2] - 16) < 90)
                    {
                        _proc_monster_wake = 1;
                        break;
                    }
                }
            }
    ...
    if (_proc_monster_wake)
    {
        state = 1;
        statetime = 0;
        if (proc_generated && instance_exists(oClient) && oClient.connected)
        {
            proc_net_owner = global.clientID;
            proc_net_timer = 0;
        }
...
if (state == 3)
{
    var _drain_valid = instance_exists(oCharacter);
    if (_drain_valid && global.playerhealth <= 0)
    {
        _drain_valid = 0;
    }
    if (_drain_valid && proc_generated && global.proc_active && instance_exists(oClient) && oClient.connected)
    {
        if (proc_drain_target != global.clientID)
        {
            _drain_valid = 0;
        }
    }
...
if (proc_generated && proc_spawn_index >= 0 && state >= 1 && global.proc_active && instance_exists(oClient))
{
    if (oClient.connected && proc_net_owner == global.clientID)
    {
        proc_net_timer -= 1;
        if (proc_net_timer <= 0)
        {
            ...
            buffer_write(_proc_monster_buffer, buffer_s16, round(hp));
            buffer_write(_proc_monster_buffer, buffer_u8, 0);
